 ...
 */
@Service
public class ApplicationService implements CoreEntityServiceInterface<applications, Integer, ApplicationsPostRequest, ApplicationPatchRequest> {
     ...
     */
    @Override
    public applications read(Integer applicationId) throws Exception {
        try {
     ...
     * </ul>
     *
     * @param request chứa jobId và thông tin ứng viên
     * @throws Exception khi xảy ra lỗi xác minh hoặc database
     */
    @Override
    public void create(ApplicationsPostRequest request) throws Exception {
        try {
            //Lấy primary CV của người dùng
            CV CandidateCV = cvRepo.getPrimary(request.getApplicateCandidateId());

            //Thêm job mới vào repository
            applicationsRepo.create(new applications(
                    request.getApplicateCandidateId(),
                    request.getJobId(),
                    CandidateCV.getCvId()
            ));

            //Tăng quantity ứng tuyển cho jobID
            jobRepo.IncreseQuantity(request.getJobId());
        } catch (BusinessException e) {
            log.error("Yêu cầu tạo đơn ứng tuyển cho người dùng {} không thành công do {}", request.getEmail(), e.getMessage());
            throw new BusinessException(e.getMessage());
        } catch (Exception e) {
            StackTraceElement origin = e.getStackTrace()[0];
            log.error("Yêu cầu tạo đơn ứng tuyển cho người dùng {} không thành công do {}. Source location: {}.{}.{}", request.getEmail(), e.getMessage(), origin.getClassName(), origin.getMethodName(), origin.getLineNumber());
            throw new Exception(e);
        }
    }
     ...
     */
    @Override
    public void update(ApplicationPatchRequest request, Integer applicationID) throws Exception {
        try {
